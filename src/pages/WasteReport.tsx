...
import { Download, Filter, Calendar, Building2, Package, Search, Eye, X, Printer, Trash2 } from 'lucide-react';
...
export default function WasteReport() {
  ...
  const filteredReports = inventoryReports.filter(r => {
    ...
    // Only include reports that have items with waste > 0
    const hasWaste = r.items.some(item => item.waste > 0);

    return dateMatch && branchMatch && permissionMatch && hasWaste;
  }).map(report => {
    const branchName = branches.find(b => b.id === report.branchId)?.name || 'غير معروف';
    const itemsWithWaste = report.items
    ...
    return {
      ...
      branchName,
      itemsWithWaste,
      totalItems: itemsWithWaste.length,
      totalQuantity: itemsWithWaste.reduce((sum, item) => sum + item.waste, 0)
    };
  }).filter(report => 
  ...
  const handleExportXLSX = () => {
    const data = filteredReports.flatMap(r => r.itemsWithWaste.map(item => ({
      'التاريخ': r.date,
      'الفرع': r.branchName,
      'مجموعة المنتج': item.category,
      'الصنف': item.itemName,
      'الوحدة': item.unit,
      'كمية الهدر': item.waste
    })));
    exportToXLSX(data, `تقرير_الهدر_${format(new Date(), 'yyyy-MM-dd')}`);
  };

  const handleExportPDF = () => {
    const headers = ['التاريخ', 'الفرع', 'مجموعة المنتج', 'الصنف', 'الوحدة', 'كمية الهدر'];
    const data = filteredReports.flatMap(r => r.itemsWithWaste.map(item => [
      r.date,
      r.branchName,
      item.category,
    ...
    exportToPDF(headers, data, `تقرير_الهدر_${format(new Date(), 'yyyy-MM-dd')}`, 'تقرير الهدر');
  };

  const handlePrintWasteReport = (report: any) => {
    const branch = branches.find(b => b.id === report.branchId);
    const itemsWithWaste = report.itemsWithWaste || report.items.filter((i: any) => i.waste > 0).map((item: any) => ({
        ...item,
        itemName: inventoryItems.find(i => i.id === item.itemId)?.name || 'صنف محذوف',
        category: inventoryItems.find(i => i.id === item.itemId)?.category || '-',
        unit: inventoryItems.find(i => i.id === item.itemId)?.unit || '-'
    ...
    const content = `
      <div dir="rtl" style="font-family: sans-serif; padding: 20px;">
        <h1 style="text-align: center; color: #ef4444;">تقرير الهدر</h1>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
          <thead>
            <tr style="background-color: #f8fafc;">
              ...
              <th style="border: 1px solid #e2e8f0; padding: 12px; text-align: right;">كمية الهدر</th>
              <th style="border: 1px solid #e2e8f0; padding: 12px; text-align: right;">السبب / ملاحظات</th>
            </tr>
          </thead>
          <tbody>
            ${itemsWithWaste.map((item: any) => `
                <tr>
                  <td style="border: 1px solid #e2e8f0; padding: 12px;">${item.itemName}</td>
                  <td style="border: 1px solid #e2e8f0; padding: 12px;">${item.unit}</td>
                  <td style="border: 1px solid #e2e8f0; padding: 12px; font-weight: bold; color: #ef4444;">${item.waste}</td>
                  <td style="border: 1px solid #e2e8f0; padding: 12px;"></td>
                </tr>
              `).join('')}
            ${itemsWithWaste.length === 0 ? '<tr><td colspan="4" style="text-align: center; padding: 20px;">لا يوجد هدر مسجل</td></tr>' : ''}
          </tbody>
        </table>

        <div style="margin-top: 50px; display: flex; justify-content: space-between;">
          <div style="text-align: center; width: 200px;">
            <p style="border-bottom: 1px solid #000; padding-bottom: 40px;">توقيع المحضر</p>
          </div>
          <div style="text-align: center; width: 200px;">
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">تقارير الهدر</h1>
              ...
              {filteredReports.map((report, index) => (
                <tr key={`${report.id}-${index}`} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm">
                    <span className="px-3 py-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-full font-bold">
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-gray-500 dark:text-slate-400">
                    <div className="flex flex-col items-center gap-2">
                      <Trash2 size={40} className="text-gray-200 dark:text-slate-800" />
                      <p>لا توجد تقارير هدر مسجلة ضمن الفلاتر المحددة</p>
                    </div>
                  </td>
                </tr>
              )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm print:bg-white print:absolute print:inset-0">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-3xl shadow-2xl border border-gray-100 dark:border-slate-800 animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh] print:shadow-none print:border-none print:max-w-none print:h-auto print:max-h-none">
            ...
            <div className="hidden print:block p-8 text-center border-b border-gray-200 mb-6">
              <h1 className="text-2xl font-bold mb-2 text-red-600">تقرير هدر</h1>
            ...
            <div className="p-6 overflow-y-auto flex-1 print:overflow-visible">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 print:grid-cols-3">
                ...
                <div className="bg-gray-50 dark:bg-slate-800/50 p-4 rounded-2xl print:bg-gray-50 print:border print:border-gray-200">
                  ...
                  <span className="text-xl font-bold text-red-600 dark:text-red-400">{selectedReport.totalItems}</span>
                </div>
              </div>

              <h3 className="font-bold text-gray-900 dark:text-white mb-4">قائمة الأصناف المهدرة</h3>
              <div className="border border-gray-100 dark:border-slate-800 rounded-2xl overflow-hidden print:border-gray-300">
                <table className="w-full text-right">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-slate-800 print:divide-gray-200">
                    {selectedReport.itemsWithWaste.map((item: any, idx: number) => (
                      <tr key={idx}>
                        ...
                        <td className="px-4 py-3 text-sm font-bold text-red-600 dark:text-red-400">{item.waste}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="p-6 border-t border-gray-100 dark:border-slate-800 flex justify-end gap-3 print:hidden">
              <button 
                onClick={() => handlePrintWasteReport(selectedReport)}
                className="px-6 py-2 bg-red-600 text-white rounded-xl hover:bg-red-700 font-bold flex items-center gap-2 shadow-lg shadow-red-500/20"
              >
                <Printer size={18} />
                طباعة التقرير
              </button>
